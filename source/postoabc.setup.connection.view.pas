...
begin
  ...
  begin
    Application.MessageBox('Conexão realizada com sucesso', 'Teste de Conexão');
    if btnSalvar.CanFocus then
      btnSalvar.SetFocus;
  end
  else begin
    Application.MessageBox('Não foi possível conectar ao banco de dados com essas configurações, verifique novamente!', 'Teste de Conexão');
